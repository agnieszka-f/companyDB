...
router.post('/employees', async (req, res) => {
  ...
  try{
    ...
    res.json({message:'A new employee has been created', newEmployee});
  } catch(err) {
    res.status(500).json({message: err});
  }
});
...
router.put('/employees/:id', async (req, res) => {
  ...
  try{
    ...
    if(emp){
      ...
      res.json({message:'The employee was updated', beforeUpdate:emp, afterUpdate: await Employee.findById(req.params.id)});
